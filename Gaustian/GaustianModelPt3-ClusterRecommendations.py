...
for cluster_num in range(0, 6):
    ...
    included_reviewers = reviewer_clusters[reviewer_clusters['cluster'] == cluster_num]
    print(included_reviewers.head())
    for reviewer in included_reviewers['review_id'].tolist():
    ...
    for product in cluster_set.keys():
        ...
        if curr_prod['num_Of_Reviews'] > 45:
    ...
    print(sorted_cluster_df.head(25))
    cluster_data[cluster_num] = sorted_cluster_df.head(25).to_dict(orient='records')

with open("cluster_recommendations", "w") as file:
